...
def validate_rut(rut: str) -> dict:
    ...
    if len(clean) < 2:
        return build_error(error="RUT demasiado corto", steps=steps)
    ...
    steps.append({
        ...
        "explanation": f"Cuerpo: {body}   |   DV ingresado: {dv_input}",
    })
    ...
    for i, digit in enumerate(reversed_digits):
        ...
        product = digit * m
        total  += product
        detail_lines.append(f"{digit} × {m} = {product}")

    steps.append({
        "title": "Algoritmo módulo 11 — productos",
        "explanation": "   ".join(detail_lines),
    })
    ...
    steps.append({
        "title": "Suma y cálculo del DV esperado",
        "explanation": (
            f"Suma total = {total}   |   "
            f"{total} mod 11 = {remainder}   |   "
            f"11 − {remainder} = {result}"
        ),
    })

    dv_expected = "0" if result == 11 else ("K" if result == 10 else str(result))

    steps.append({
        "title": "Comparación de dígitos verificadores",
        "explanation": f"DV esperado: {dv_expected}   |   DV ingresado: {dv_input}",
        "result": " Coinciden" if dv_expected == dv_input else "✗ No coinciden",
    })
    ...
    return build_success(
        ...
        data={
            ...
            "body":         body,
            "digits":       digits,
            "named_digits": {f"d{i+1}": digits[i] for i in range(8)},
            "dv_input":     dv_input,
            "dv_expected":  dv_expected,
            "v":            v,
        },
